...
mod tests {
    ...
    #[test]
    fn resolve_repo_path_uses_schema_default_when_present() {
        let schema = json!({
            "properties": {
                "workspace": {
                    "properties": {
                        "repoPath": {
                            "default": "/tmp/demo-repo"
                        }
                    }
                }
            }
        ...
        assert_eq!(resolve_repo_path(&schema), "/tmp/demo-repo");
    }

    #[test]
    fn resolve_repo_path_falls_back_to_current_dir_when_missing() {
        let schema = json!({});
